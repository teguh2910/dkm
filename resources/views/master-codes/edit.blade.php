...
@section('title', 'Edit Master Code')

@section('content')
    <div class="card">
        <div style="margin-bottom: 24px;">
            <h2 style="font-size: 20px; font-weight: 600; color: #1f2937;">Edit Master Code</h2>
        </div>

        <form action="{{ route('master-codes.update', $masterCode) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div style="margin-bottom: 16px;">
                ...
                <input type="text" id="code" name="code" value="{{ old('code', $masterCode->code) }}" required
            ...
            <div style="margin-bottom: 16px;">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $masterCode->name) }}" required
            ...
            <div style="margin-bottom: 16px;">
                ...
                <textarea id="description" name="description" rows="3" placeholder="Deskripsi master code (opsional)"
                    style="display: block; width: 100%; padding: 8px 12px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 14px;">{{ old('description', $masterCode->description) }}</textarea>
                @error('description')
                    <span style="color: #ef4444; font-size: 14px; margin-top: 4px; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 16px;">
                ...
                <input type="number" id="year" name="year" value="{{ old('year', $masterCode->year) }}" required
                    min="2000" max="2100"
                    style="display: block; width: 100%; padding: 8px 12px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 14px;">
                @error('year')
                    <span style="color: #ef4444; font-size: 14px; margin-top: 4px; display: block;">{{ $message }}</span>
