...
@section('title', 'Edit Pengeluaran Kas Kecil')

@section('content')
    <div class="card">
        <div style="margin-bottom: 24px;">
            <h2 style="font-size: 20px; font-weight: 600; color: #1f2937;">Edit Pengeluaran Kas Kecil</h2>
        </div>

        <form action="{{ route('cash-expenses.update', $cashExpense) }}" method="POST" id="cashExpenseForm">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="date" name="tanggal" id="tanggal" class="form-control"
                    value="{{ old('tanggal', $cashExpense->tanggal->format('Y-m-d')) }}" required>
                @error('tanggal')
                    <div class="text-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="dibayarkan_kepada" id="dibayarkan_kepada" class="form-control"
                    value="{{ old('dibayarkan_kepada', $cashExpense->dibayarkan_kepada) }}"
            ...
            <div class="form-group">
                ...
                <input type="number" name="sebesar" id="sebesar" class="form-control"
                    value="{{ old('sebesar', $cashExpense->sebesar) }}" placeholder="0" step="0.01" min="0"
                    required>
                @error('sebesar')
                    <div class="text-error">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group">
                ...
                <textarea name="keterangan2" id="keterangan2" class="form-control" rows="3"
                    placeholder="Keterangan tambahan (opsional)">{{ old('keterangan2', $cashExpense->keterangan2) }}</textarea>
                @error('keterangan2')
                    <div class="text-error">{{ $message }}</div>
                @enderror
            </div>
